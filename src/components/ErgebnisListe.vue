<template>
  <div class="ergebnisListe">
    <div id="inhalt" class="card-deck">
      <div v-for="jahr in jahre" class="col-xl-10 offset-xl-1 col-sm-12" :key="jahr.jahr">
  ...
  methods: {
    ...
    fetchData() {
      ...
      if (this.jahre.length === 0) {
        axios
          .get("static/data/aktuell/newsdate.json")
          .then(response => {
            this.jahre = [];
            for (let i = 0; i < response.data.length; i++) {
              ...
              jahr.termine = response.data[i].termine.sort((a, b) => {
                return (
                  new Date(this.convDate(a.datum)) -
                  new Date(this.convDate(b.datum))
                );
              });
    ...
    convDate(datum) {
      // doublette @todo Auslagern nach vuex
      if (datum) {
